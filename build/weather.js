const autoLocateHandler = (function () {
  function sendPosition(position) {
    ...
    fetch(queryString, { method: "GET" })
      ...
      .then((result) => {
        ...
        let tempUnit = document.getElementById("temp-unit-checkbox").checked
          ? "fahrenheit"
          : "celsius";
        instance.initiateWeatherDemo({
          latitude: position.coords.latitude,
          longitude: position.coords.longitude,
          city: result.features[0].properties.city,
          province:
            result.features[0].properties.province ||
  ...
  return function () {
    ...
    if (navigator.geolocation) {
      ...
      alert(
        "This browser does not support autolocation. Use search bar instead",
      );
    }
  };
})();
    ...
    promise.then(
      (res) => {
        ...
        for (let i = 0; i < items; i++) {
          let location = `${res.results[i].name}${
            res.results[i].admin1 ? ", " + res.results[i].admin1 : ""
          }${res.results[i].country ? ", " + res.results[i].country : ""}`;
  ...
  function init() {
    ...
    function todayWeather(res, date) {
      ...
      document.getElementById("today-temp").innerHTML = `${Math.round(
        res.weather.current_weather.temperature,
      )}&#176`;

      document.getElementById(
        "today-weather-summary",
      ).style.backgroundImage = `url(assets/pics/${weatherState[2]}.jpg)`;

      document.getElementById(
        "today-weather-icon",
      ).style.backgroundImage = `url(assets/icons/${weatherState[1]}.svg)`;
      ...
      for (let i = 0; i < 10; i++) {
        ...
        child.innerHTML = `<i class="bi bi-${detailIcons[i]}"></i>
    ...
    function hourlyWeather(res, date) {
      ...
      for (let i = spanStart; i <= spanEnd; i++) {
        ...
        parent.innerHTML = `<span class="forecast-element-time">${time}</span>
          <i class="forecast-element-icon hourly-icon"></i>
          <span class="forecast-element-temp">${Math.round(
        ...
        document.querySelectorAll(".hourly-icon")[
          i - spanStart
        ].style.backgroundImage = `url(assets/icons/${weatherState[1]}.svg)`;
        ...
        let detailsParent =
          document.querySelectorAll(".hourly-detail")[i - spanStart];
        for (let j = 0; j < 6; j++) {
          let child = document.createElement("DIV");
          child.setAttribute("class", "forecast-element-detail-item");
          child.innerHTML = `<i class="bi bi-${detailIcons[j]} detail-icon"></i>
        ...
        parent.addEventListener(
          ...
          (function () {
            let close = true;
            let arrow =
              document.querySelectorAll(".hourly-arrow")[i - spanStart];
            let elem =
              document.querySelectorAll(".hourly-detail")[i - spanStart];
            return function () {
              if (close) {
    ...
    function dailyWeather(res) {
      ...
      for (let i = 0; i < dailyData.time.length; i++) {
        ...
        parent.setAttribute("class", "forecast-element");
        parent.innerHTML = `<span class="forecast-element-time">${demoDate}</span>
          <i class="forecast-element-icon daily-icon"></i>
          <span class="forecast-element-temp">${Math.round(
            dailyData.temperature_2m_max[i],
          )}&#176/${Math.round(dailyData.temperature_2m_min[i])}&#176</span>
          <span class="forecast-element-status">${weatherState[0]}</span>
        ...
        document.querySelectorAll(".daily-icon")[
          i
        ].style.backgroundImage = `url(assets/icons/${weatherState[1]}.svg)`;
        ...
        let detailValues = [
          ...
          new Date(dailyData.sunrise[i]).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
          new Date(dailyData.sunset[i]).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
        ];
    ...
    return {
      ...
      initiateWeatherDemo: function (locationData) {
        ...
        ).innerHTML = `${locationData.city} weather as of ${time}`;

        let startDate = `${date.getFullYear()}-${(
          "0" +
          (date.getMonth() + 1)
        ).slice(-2)}-${("0" + date.getDate()).slice(-2)}`;
        let endDate = `${date2.getFullYear()}-${(
          "0" +
          (date2.getMonth() + 1)
        ...
        promise.then((res) => {
          ...
          document.getElementById("navbar").childNodes[3].addEventListener(
            "click",
            (function () {
          ...
          document.getElementById("navbar").childNodes[5].addEventListener(
            "click",
            (function () {
              dailyWeather(res);
              return function () {
...
function weatherStatusDemo(code, isDay) {
  // no need to redefine weatherCodes const every time the function is called
  const weatherCodes = {
    0: [
      "Clear",
      isDay ? "sun-fill" : "moon-stars-fill",
      isDay ? "clear_day" : "clear_night",
    ],
    1: [
      "Mainly clear",
      isDay ? "sun-fill" : "moon-stars-fill",
      isDay ? "mainly_clear_day" : "partly_cloudy_night",
    ],
    2: [
      "Partly cloudy",
    ...
    86: [
      "Heavy snow shower",
      "cloud-snow",
      isDay ? "snowy_day" : "snowy_night",
    ],
    95: [
      "Thunderstorm",
      "cloud-lightning-fill",
      isDay ? "thunder_day" : "thunder_night",
    ],
...
document.getElementById("temp-unit-container").addEventListener("click", () => {
  let instance = weatherResults.getInstance();
  let unit = document.getElementById("temp-unit-checkbox").checked
    ? "celsius"
    : "fahrenheit";
  let locData = JSON.parse(localStorage.getItem("weatherLatestLocData"));
